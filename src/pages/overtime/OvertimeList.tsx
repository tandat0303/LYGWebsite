...
function groupByMonth(items: OvertimeData[]): Map<number, OvertimeData[]> {
  const map = new Map<number, OvertimeData[]>();
  for (const item of items) {
    const month = getMonth(item.Check_Day);
  ...
  for (const [, list] of map) {
    list.sort(
      (a, b) =>
        parseDate(a.Check_Day).getTime() - parseDate(b.Check_Day).getTime(),
    );
  }
  return map;
}
...
function YearPicker({ year, onChange }: YearPickerProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <span className="text-orange-500 dark:text-orange-400">{year}</span>
        <svg
          width="10"
          height="10"
          viewBox="0 0 24 24"
      ...
      {open && (
        <div
          className="absolute top-[calc(100%+6px)] right-0 z-50 rounded-2xl border
            ...
            backdrop-blur-xl
            animate-[ot-drop_0.16s_cubic-bezier(0.22,1,0.36,1)]"
...
function MonthAccordion({
  ...
  const totalHours = items.reduce((acc, i) => acc + i.Overtime, 0);
    ...
    >
      ...
      >
        <div
          className="w-9 h-9 rounded-xl shrink-0 flex items-center justify-center font-bold text-[13px]
            bg-orange-500/10 dark:bg-orange-500/18 text-orange-600 dark:text-orange-400
            border border-orange-400/20 dark:border-orange-400/18"
        >
          {month}
        </div>
        ...
        <div className="flex-1 text-left min-w-0">
          ...
          <p className="text-[11.5px] m-0 text-slate-400/80 dark:text-white/35">
            {totalDays} {t("ngay")} · {totalHours} {t("gio")}
          </p>
        </div>
      ...
      >
        <div className="border-t border-slate-100 dark:border-white/5">
          {items.map((item, i) => (
            <OvertimeItem
              key={`${item.Person_ID}-${item.Check_Day}-${i}`}
              item={item}
