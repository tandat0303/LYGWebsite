...
import { Timer, TrendingUp, CalendarDays, Clock } from "lucide-react";

const MAX_OVERTIME_HOURS = 300;
...
function SkeletonCard() {
  return (
    ...
    >
      <div className="h-4 w-40 rounded bg-slate-200 dark:bg-white/8 mb-5" />
      <div
...
export default function OvertimeSummaryCard({ data, year, loading }: Props) {
  if (loading) return <SkeletonCard />;

  const totalHours = data.reduce((acc, item) => acc + item.Overtime, 0);
  const totalDays = data.length;
  const remainingHours = Math.max(0, MAX_OVERTIME_HOURS - totalHours);
  ...
  return (
    ...
    >
      ...
      >
        <style>{`
          @media (max-width: 640px) { .ot-summary-grid { grid-template-columns: repeat(2, minmax(0,1fr)) !important; } }
        `}</style>
        <div className="ot-summary-grid contents">
          <StatItem
            label="lamThemToiDa"
            value={MAX_OVERTIME_HOURS}
            sub="gio"
            accent="orange"
