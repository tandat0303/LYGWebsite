...
export interface NonTaiwanSummary {
  // Home Leave
  ...
  transport_limitPerYear: number; // e.g. 5
  // Each entry = a month when transport was used (or upcoming)
  transport_usedMonths: string[]; // e.g. ["MAR"]
  // All available slots (ordered months shown in timeline)
  transport_timelineMonths: string[]; // e.g. ["MAR","MAY","JUL","SEP","DEC"]
}
...
function HomeLeaveSec({ data }: { data: NonTaiwanSummary }) {
  return (
...
// ─── Annual Transportation Allowance section ──────────────────────────────────

function TransportSec({ data }: { data: NonTaiwanSummary }) {
  const usedSet = new Set(
    data.transport_usedMonths.map((m) => m.toUpperCase()),
  );

  return (
    <SectionCard title="Annual Transportation Allowance">
      {/* Limit label */}
      ...
      >
        ...
        {data.transport_timelineMonths.map((month) => {
          const used = usedSet.has(month.toUpperCase());
          return (
            <div
              key={month}
              style={{
