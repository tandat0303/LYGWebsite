...
import TransportAllowanceCard from "./TransportAllowanceCard";
...
export default function SupervisorLeaveNTW({
  year,
  onYearChange,
  status,
}: Props) {
  ...
  const [subsidyData, setSubsidyData] = useState<DataSubsidy[] | null>(null);
  ...
  useEffect(() => {
    const fetchSubsidy = async () => {
      try {
        ...
        const res = await supervisorLeaveApi.getSubsidyData(payload);
        if (res.status) setSubsidyData(res.dataSubsidy);
      } catch (err) {
        AppAlert({ icon: "error", title: getApiErrorMessage(err) });
      } finally {
        setLoadingSubsidy(false);
      }
    };
    fetchSubsidy();
  }, [year]);
  ...
  return (
    <>
      <style>{`
        ...
        .ntw-page { animation: ntw-in 0.28s cubic-bezier(0.22,1,0.36,1); }
      `}</style>

      <div className="ntw-page w-full min-h-full box-border p-6 overflow-x-hidden max-sm:p-4">
        {/* Header */}
        ...
        <div className="grid grid-cols-2 max-sm:grid-cols-1 gap-3.5 mb-5">
          <HomeLeaveQuota
            data={leaveInfo}
            loading={loadingInfo}
            status={status}
          />
          <TransportAllowanceCard data={subsidyData} loading={loadingSubsidy} />
        </div>

        {/* <div className="flex items-center gap-2 mb-3.5">
          <div className="w-1.5 h-1.5 rounded-full bg-slate-800/20 shrink-0 dark:bg-white/25" />
          <span className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-700/40 dark:text-white/40">
            Leave History
          </span>
        </div> */}
